...
class Lazerdom(object):
    ...
    def visualize_residuals_2(self,res_func,guess_del):
        x,y,theta = guess_del
        xs = [x+.1*i for i in range(-20,20)]
        ys = [y+.1*i for i in range(-20,20)]

        rs = [[res_func(np.array([[x_i], [y_i], [theta]])) for y_i in ys] for x_i in xs]
        rs = np.array(rs)

        pp.contourf(xs,ys,rs, label = "residual")
        pp.title("Varying x and y")
        pp.legend()
        pp.show()


    def visualize_residuals(self,res_func,guess_del):
        x,y,theta = guess_del
        xs = [x+.01*i for i in range(-200,200)]
        vals = [np.array([[x_i], [y], [theta]]) for x_i in xs]
        rs = [res_func(val) for val in vals]
        ...
        ys = [y+.01*i for i in range(-200,200)]
        vals = [np.array([[x], [y_i], [theta]]) for y_i in ys]
        rs = [res_func(val) for val in vals]
        ...
        ts = [theta+.01*i for i in range(-200,200)]
        vals = [np.array([[x], [y], [t_i]]) for t_i in ts]
        rs = [res_func(val) for val in vals]

        pp.subplot(133)
        pp.plot(xs,rs)
        pp.title("Varying Theta")
        pp.xlabel("theta value")
        pp.ylabel("residual")
        pp.show()
    ...
    def execute(self):
        ...
        while not(rospy.is_shutdown()):
            ...
            if old != None and new != None and guess_del != None:
                ...
                self.visualize_residuals_2(res_func,guess_del)
